...
('lucky', "강해원", "미국 뉴욕", "01012345678", "2002-03-02");

insert into customer (addr, phone, birth, custid, custname)values
('대한민구  부산', '01022334455', '2020-03-03', 'wow', '이지은');

-- 여러 튜플 동시 추가
insert into customer values ('lucky', "강해원", "미국 뉴욕", "01012345678", "2002-03-02"),
('luy', "강원", "미국 뉴욕", "01012345678", "2002-03-02"),
('lucy', "강해원", "미국 뉴욕", "01012345678", "2002-03-02");
...
-- 사전순으로 밧민지보다 뒤에 위치한 고객의 모든 정보 검색
select * from customer where custname > '박민지';

-- 범위
-- 1950년 이상 2000년 이하 출생 고객 검색
select * from customer where birth between '1995-01-01' and '2000-12-31';
select * from customer where birth >= '1995-01-01' and birth <= '2000-12-31'; -- 동일한 값임
...
-- where 절과 order by 함께 사용 (이떄, where보다 order by가 뒤에 위치해야 함)
...
-- 주문 테이블 에서 총 판매 개수와, 평균 판매 개수, 상품 최저가, 상품 최고가 검색

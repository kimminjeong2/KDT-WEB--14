...
-- 테이블 : 하나 이상의 열과 행으로 구성된 데이터베이스 최소 단위
-- SQL : DBMS 에서 사용되는 언어 (데이터베이스를 구축, 관리, 활용)
...
-- 제약조건("옵션")
-- NOT NULL : NULL 허용 x
-- AUTO_INCREMENT : 자동 숫자 증가, 테이블에 데이터 추가될 때마다
-- PRIMARY : 기본키 (중복값 허용 x, NULL 허용x) -> 하나의 테이블당 하나만!
...
-- 3. 테이블 구조 확인
-- 테이블의 컬럼 정보 (자료형, NULL 여부, KEY, DEFAULT)
DESC product;
